   ...
   <div class="col-12 col-sm-6 col-md-12">
      ...
      <div class="card" style="margin-left: 10px;">
              <div class="card-header border-transparent">
                <h3 class="card-title">SILVER PLAN MEMBERS</h3>
...
if (isset($_POST['submit'])) {
     ...
     $q = mysqli_real_escape_string($conn,$_POST['q']);
      ...
      $sql="SELECT * FROM user WHERE `$column` LIKE '$q'";
      $result = $conn->query($sql);
                    ...
                    if($result -> num_rows > 0){
                     ...
                     <td>" . $row["regdate"]. "</td> <td>" . $row["plan"]. "</td> <td>" . $row["state"]. "</td> <td>" . $row["refer"]. "</td>  <td>" . $row["aadhaar"]. "</td> 
                      <td>" . $row["panno"]. "</td> <td>" . $row["accountno"]. "</td> <td>" . $row["lastlogin"]. "</td> <td>" . $row["user_status"]. "</td> <td> <a href=\"block.php?id=$row[id]\">BLOCK</a> </td>  <td> <a href=\"delete.php?id=$row[id]\">DELETE</a> </td>  </tr>";
          }
                           }else{
            echo "<script>alert('NO RECORDS FOUND');document.location='showuser.php'</script>";
                           }
                         }
?>
