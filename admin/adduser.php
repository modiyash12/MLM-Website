...
$val=$_SESSION["memberid"];
  ...
  if (isset($_POST['submit'])) {
      ...
      $refer = mysqli_real_escape_string($conn,$_POST['refer']);
       ...
       $sql = "INSERT INTO `user` ( `name`, `email`, `mobileno`, `address`, `memberid`, `password`, `regdate`,`plan`, `state`,`refer`,`aadhaar`, `panno`,`accountno`,`user_status`) VALUES ('$name','$email',$mobileno,'$address','$memberid','$password',now(),$plan, '$state' ,'$refer' ,0, 0, 0,'ACTIVATED' )";
 

if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Record Sucessfully Inserted');document.location='admindashboard.php'</script>";
        } else {
