...
$val=$_SESSION["memberid"];
  ...
  if (isset($_POST['submit'])) {
      ...
      $refer = mysqli_real_escape_string($conn,$_POST['refer']);
       ...
       $sql = "INSERT INTO `user_g` ( `name`, `email`, `mobileno`, `address`, `memberid`, `password`, `regdate`,`plan`, `state`,`refer`,`aadhaar`, `panno`,`accountno`,`user_status`) VALUES ('$name','$email',$mobileno,'$address','$memberid','$password',now(),$plan, '$state' ,'$refer' ,0, 0, 0,'PENDING' )";
 

if ($conn->query($sql) === TRUE) {
            echo "<script>alert('Record Sucessfully Inserted');document.location='user_dashboard2.php'</script>";
        } else {
            echo "<script>alert('Record  Inserting failed');document.location='activate_user2.php'</script>";
            }
...
include('header.php');
    ...
    $sql="SELECT count(epin) AS total FROM `epin_list_g` WHERE `memberid`='$id' AND `status`='UNUSED' ";
   ...
   <div class="col-md-12">
            ...
            <div class="card card-warning">
              ...
              <!-- /.card-header -->
              <div class="card-body">
                <form action="activate_user2.php" method="POST">
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-6">
                      <div class="form-group">
                        <label>EMAIL</label>
                        <input type="email" class="form-control" placeholder="Enter Email" value="null@g" name="email" >
                      </div>
                    </div>
