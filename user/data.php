...
$sql="SELECT count(id) AS total from user WHERE `refer`='$val'";
...
$sql="SELECT count(id) AS total from user WHERE `aadhaar`= 0 AND `refer`='$val'";
...
$sql="SELECT count(id) AS total from `epin_list` WHERE `status`='UNUSED' AND `memberid`='$val' ";
...
$sql="SELECT count(id) AS total from user WHERE `refer`= '$val' AND `user_status`='PENDING'";
